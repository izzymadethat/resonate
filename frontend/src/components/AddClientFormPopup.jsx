...
import { fetchAddClient } from "../store/clients";
import { fetchProject, fetchProjects } from "../store/projects";

const AddClientFormPopup = ({ projectId }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        return dispatch(fetchAddClient(clientData))
            .then(() => {
                closeModal();
    ...
    return (
        <div className="relative w-full p-8 space-y-6 lg:max-w-md min-h-80">
            <h3 className="text-2xl font-bold">Add a new client</h3>
            <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="absolute flex gap-4 text-sm bottom-4 right-4">
                    ...
                    <button className="p-2 rounded-md bg-primary hover:bg-primary/80 disabled:bg-primary/50 text-neutral-800" disabled={submitting || !clientName || !clientEmail}>{submitting ? "Adding Client..." : "Add New Client"}</button>
                </div>
            </form>
        </div>
    );
};
export default AddClientFormPopup;