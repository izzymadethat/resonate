...
const FileUploader = ({ className, projectId }) => {
    ...
    const removeRejectedFile = (fileName) => {
        setRejected((prevRejected) =>
            prevRejected.filter(({ file }) => file.name !== fileName)
        );
    };
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmitFiles}>
            {(files.length > 0 || rejected.length > 0) && (
                <div className="flex items-center justify-end gap-4 lg:justify-start">
                    <button
                        type="button"
                        className={"px-2 py-1 text-sm rounded-md bg-primary hover:bg-primary/90 text-neutral-800"}
                    ...
                    <button
                        ...
                        className={
                            "p-2 text-sm border rounded-md border-primary text-neutral-100 hover:bg-primary hover:text-neutral-900 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-neutral-100"
                        }
                    >
                        {isUploading ? (
                            ...
                            <div className="flex items-center gap-2">
                                <CloudUpload className="size-4" /> <span>Upload</span>
                            </div>
                        )}
                    </button>
                </div>
            ...
            {errors.error && (
                <div className="flex items-center justify-center gap-2">
                    {" "}
                    <span className="text-red-600 size-6">
                        <AlertTriangle />
                        {errors.error}
                    </span>
                </div>
            ...
            >
                ...
                {isDragActive ? (
                    ...
                    <div className="flex flex-col items-center justify-center w-full py-6 space-y-2 rounded-md cursor-pointer hover:bg-neutral-500/20">
                        ...
                        <p className="text-xs">
                            (Only audio files are allowed. Files must be less than 250MB)
                        </p>
                    </div>
                )}
            </div>
            ...
            {files.length > 0 && (
                <div>
                    <h3 className="font-semibold text-center lg:text-left lg:text-lg text-neutral-100">
                        File{files.length > 1 && "s"} to be uploaded
                    </h3>
                    {files.length > 4 && (
                        <button
                            type="button"
                            className="px-2 py-1 text-sm rounded-md bg-primary hover:bg-primary/90 text-neutral-800"
                            onClick={() => toggleShowAllFiles(setShowAllFiles)}
                        >
                        ...
                        {showAllFiles ? (
                            <>
                                {files.map((file) => (
                                    <li
                                        key={file.name}
                                        className="relative w-full h-full p-6 transition-transform duration-200 rounded-md cursor-pointer bg-neutral-800 hover:scale-95"
                                    >
                                        <AudioLines className="text-primary lg:size-10" />
                                        <button
                                            type="button"
                                            className="absolute z-30 p-1 rounded-full hover:bg-red-600 top-2 right-2"
            ...
            {rejected.length > 0 && (
                <div>
                    <h3 className="font-bold text-center text-red-600 lg:text-left lg:text-lg">
                        File{rejected.length > 1 && "s"} not accepted
                    </h3>
                    {rejected.length > 4 && (
                        <button
                            type="button"
                            className="px-2 py-1 text-sm rounded-md bg-primary hover:bg-primary/90 text-neutral-800"
                            onClick={() => toggleShowAllFiles(setShowAllRejected)}
                        >
                            {showAllRejected ? "Show less" : "Show all"}
                        </button>
                    )}
                            ...
                            <>
                                {rejected.slice(0, 3).map(({ file, errors }) => (
                                    ...
                                    >
                                        ...
                                        <ul>
                                            {errors.map((err) => (
                                                <li key={err.code} className="text-xs text-red-600">
                                                    {parseFileErrorMessage(err)}
                                                </li>
                                            ))}
                                        </ul>
                                    </li>
